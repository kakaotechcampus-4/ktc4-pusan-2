...
describe('writeFailures', () => {
  ...
  it('첫 건에만 true 를 돌려준다 — 호출부의 "한 번만" 이 여기 걸립니다', () => {
    expect(noteWriteFailure(SESSION, 'audioChunk', new Error('quota'))).toBe(true);
    expect(noteWriteFailure(SESSION, 'audioChunk', new Error('quota'))).toBe(false);
    expect(noteWriteFailure(SESSION, 'audioChunk', new Error('quota'))).toBe(false);
  });

  it('종류가 다르면 각각 첫 건이다', () => {
    expect(noteWriteFailure(SESSION, 'audioChunk', null)).toBe(true);
    expect(noteWriteFailure(SESSION, 'gazeDecision', null)).toBe(true);
  });

  it('종류별로 센다', () => {
    noteWriteFailure(SESSION, 'audioChunk', null);
    noteWriteFailure(SESSION, 'audioChunk', null);
    noteWriteFailure(SESSION, 'slideChange', null);

    expect(readWriteFailures(SESSION)).toEqual({ audioChunk: 2, slideChange: 1 });
  ...
  it('세션이 섞이지 않는다 — 한 탭에서 Take 를 여러 번 합니다', () => {
    ...
    expect(readWriteFailures(SESSION)).toEqual({ audioChunk: 1 });
  ...
  it('콘솔에는 첫 건만 찍는다 — 저장소가 차면 5초마다 같은 실패가 옵니다', () => {
    const spy = vi.spyOn(console, 'error').mockImplementation(() => undefined);

    noteWriteFailure(SESSION, 'audioChunk', new Error('quota'));
    noteWriteFailure(SESSION, 'audioChunk', new Error('quota'));
    noteWriteFailure(SESSION, 'audioChunk', new Error('quota'));
